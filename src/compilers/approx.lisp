...
(defun convert-su4-to-su2x2 (SU2x2)
  ...
  (let ((m (magicl:empty '(4 4) :type '(complex double-float))))
    (dotimes (block-i 2)
      (dotimes (block-j 2)
        (dotimes (inner-i 2)
          (dotimes (inner-j 2)
    ...
    (multiple-value-bind (u sigma vt) (magicl:svd m)
      (let ((a (magicl:empty '(2 2) :type '(complex double-float)))
            (b (magicl:empty '(2 2) :type '(complex double-float))))
        (dotimes (i 2)
          (dotimes (j 2)
...
(defun ensure-positive-determinant (m)
  (if (double= -1d0 (magicl:det m))
      (magicl:@ m (magicl:from-diag (list -1 1 1 1) '(4 4)
                                    :type '(complex double-float)))
      m))
      ...
      (let* ((rand-coeff (random 1.0d0))
             ...
             (evals (magicl:diag
                     (magicl:@ (magicl:transpose evecs)
                               ...
                               evecs)))
             (v (magicl:@ evecs
                    (magicl:from-diag evals (list (length evals)
                                                  (length evals)))
                    (magicl:transpose evecs))))
        (when (magicl:= gammag v +double-comparison-threshold-strict+)
          (assert (magicl:=
                   (magicl:deye #C(1d0 0d0) '(4 4))
                   (magicl:@ (magicl:transpose evecs)
                       evecs)
...
(defun orthogonal-decomposition (m)
  ...
  (let* ((m (magicl:scale m (expt (magicl:det m) -1/4)))
    ...
    (when (double~ -1d0 (magicl:det b))
      (setf d (magicl:@ d (magicl:from-diag (list -1 1 1 1) '(4 4) :type '(complex double-float))))
      (setf b (magicl:@ (magicl:from-diag (list -1 1 1 1) '(4 4) :type '(complex double-float)) b)))
    (when *compress-carefully*
      ...
      (assert (matrix-equals-dwim (magicl:deye #C(1d0 0d0) '(4 4))
